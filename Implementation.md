...
```vbscript
' Called with the UserForm_Clicked event, returns a CommandButton object when
' it is the active control or nothing. Relies on a design where primary active
' control is always a frame and the CommandButtons are in a subordinate frame.
' ----------------------------------------------------------------------------
Private Function Clicked() As Variant
   ...
   Do
      Select Case TypName(ctl)
         ...
         Case "CommandButton"
            ...
            Exit Do
         Case Else
            ' When the active control is neither a frame
            ' nor a command button something else must have been clicked
            Set Clicked = Nothing
            Exit Do
      End Select
   Loop
...
' When the clicked control is a CommanButton
' its value is returned to the caller and the 
' form is closed
' -------------------------------------------
Private Sub UserForm_Clicked
   Dim v As Variant
   Dim cmb As MsForms.CommandButtonv
   Set v = Clicked
   If TypeName(v) = "CommandButton" Then
      ' Return the Command Buttons value
      mMsg.MsgReply = dctButtons(v)
   End If
